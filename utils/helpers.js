...
export function dateStringParser(string) {
  ...
  if (time - epoch < 1000 * 60) return 'just now'
    ...
    const min = parseInt((time - epoch) / 60000)
...
export const uniqueRandomString = (length, type = 0) => {
  ...
  const randomLength = length - time.length
  for (let i = 0; i < randomLength; i++) {
    string += permittedCharacters[_.random(0, permittedCharacters.length - 1)]
  }
  const index = _.random(0, randomLength)
  const end = string.substr(index)
  const start = string.substr(0, index)
  const finalString = start + time + end
  return finalString
}
